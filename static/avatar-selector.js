...
class AvatarSelector {
    ...
    init() {
        ...
        // Don't load avatar config immediately - wait for authentication
        // Just update with default config for now
        this.updateAvatar();
        
        console.log('Avatar Selector initialized - waiting for authentication completion to load user config');
    }
    ...
    setupEventListeners() {
        ...
        if (generateRandomBtn) {
            generateRandomBtn.addEventListener('click', () => this.generateRandomAvatar());
        }
    ...
    generateRandomAvatar() {
        ...
        const hairStyles = ['ShortHairShortFlat', 'LongHairStraight', 'ShortHairShortCurly', 'LongHairBob', 'ShortHairTheCaesar'];
        const hairColors = ['Auburn', 'Black', 'Blonde', 'Brown', 'BrownDark', 'Red'];
        const clotheTypes = ['BlazerShirt', 'Hoodie', 'ShirtCrewNeck', 'BlazerSweater'];
        const clotheColors = ['Blue02', 'Gray02', 'Heather', 'PastelBlue', 'Pink', 'Red'];
        const skinColors = ['Light', 'Yellow', 'Pale', 'DarkBrown', 'Brown'];
        
        // Randomize config
        this.currentConfig.topType = hairStyles[Math.floor(Math.random() * hairStyles.length)];
        this.currentConfig.hairColor = hairColors[Math.floor(Math.random() * hairColors.length)];
        this.currentConfig.clotheType = clotheTypes[Math.floor(Math.random() * clotheTypes.length)];
        this.currentConfig.clotheColor = clotheColors[Math.floor(Math.random() * clotheColors.length)];
        this.currentConfig.skinColor = skinColors[Math.floor(Math.random() * skinColors.length)];
    ...
    /**
     * Save avatar configuration to user info
     */
    async saveAvatarConfig() {
        try {
            const fetchFunction = window.authenticatedFetch || fetch;
            
            // Get current user info to preserve it
            ...
            // Save avatar config along with existing user info
            console.log('Attempting to save avatar config:', this.currentConfig);
            const response = await fetchFunction('/api/user-info', {
                ...
                body: JSON.stringify({
                    ...
                    avatarConfig: this.currentConfig
                })
            });
            ...
            if (response.ok) {
                ...
                console.warn('Failed to save avatar configuration:', response.statusText);
                return false;
            }
        } catch (error) {
    ...
    /**
     * Load avatar configuration from user info
     */
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.avatarConfig) {
                    console.log('Found saved avatar config:', data.avatarConfig);
                    // Merge loaded config with current config (preserve any new defaults)
                    this.currentConfig = { ...this.currentConfig, ...data.avatarConfig };
                    console.log('Applied avatar config:', this.currentConfig);
                    this.updateAvatar();
                    console.log('Avatar configuration loaded successfully');
                    return true;
                } else {
                    console.log('No saved avatar configuration found');
                    return false;
                }
            } else {
                console.log('Could not load avatar configuration (user may not be authenticated yet)');
                return false;
            }
        } catch (error) {
            console.log('Avatar configuration not available:', error.message);
            return false;
        }
    }

    /**
     * Initialize avatar config after authentication is complete
     * This can be called externally once the user is authenticated
     */
    async initializeAfterAuth() {
        try {
            await this.loadAvatarConfig();
            console.log('Avatar selector initialized after authentication');
        } catch (error) {
            console.log('Could not initialize avatar after auth:', error);
        }
    }

    saveAvatar() {
        // Save avatar configuration to user database
    ...
    showVariationsPreview(variations) {
        ...
        if (!previewSection) {
            ...
            `;
            // Append to the main customization section
            const customizationSection = document.querySelector('.lg\\:col-span-2 .bg-white');
            if (customizationSection) {
                customizationSection.appendChild(previewSection);
            } else {
                // Fallback: append to main container
                document.querySelector('.max-w-6xl').appendChild(previewSection);
            }
        }
