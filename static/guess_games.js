/**
 * Guess Games Logic (Who/Where/What)
 * Mode A: Player 1 selects an item, Player 2 plays against AI with yes/no guesses
 * Mode B: Player 2 selects an item, Player 1 (AI) tries to guess through clues
 */

// Game Type Configuration - Detect from URL parameter
const urlParams = new URLSearchParams(window.location.search);
const GAME_TYPE = urlParams.get('type') || 'who'; // Default to 'who' for backward compatibility
...
const GAME_CONFIG = {
    who: {
        ...
        customCategoriesField: 'guess_who_categories',
        exampleInstruction: 'Think of a person'
    },
    where: {
        title: 'Guess Where',
        itemType: 'place',
        ...
        customCategoriesField: 'guess_where_categories',
        exampleInstruction: 'Think of a place'
    },
    what: {
        title: 'Guess What',
        itemType: 'thing',
        ...
        customCategoriesField: 'guess_what_categories',
        exampleInstruction: 'Think of a thing'
    }
};
...
// Screen history for in-game back navigation
const screenHistory = [];
...
async function initializeGame() {
    console.log(`Initializing ${CONFIG.title} game`);
...
async function loadCategories() {
    ...
    try {
        ...
        const response = await authenticatedFetch(getApiUrl('categories'), {
            method: 'POST',
...
async function startModeA() {
    ...
    try {
        gameState.isLoading = true;
        await announceText(`See if you can guess the ${gameState.selectedCategory} I am thinking of. Give me a moment to select.`, false);
        const response = await authenticatedFetch(getApiUrl('generate-people'), {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
...
async function selectPerson(person) {
    ...
    try {
        ...
        const response = await authenticatedFetch(getApiUrl('generate-clues'), {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                category: gameState.selectedCategory,
...
async function generateAndShowGuesses() {
    ...
    try {
        const response = await authenticatedFetch(getApiUrl('generate-guesses'), {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                category: gameState.selectedCategory,
...
function advanceGuessGamesScanningStep() {
...
function setupCustomCategoriesUI() {
    ...
    async function saveCustomCategories() {
        ...
        try {
            const response = await authenticatedFetch(getApiUrl('custom-categories'), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
