...
async function initializePage() {
    if (isAuthContextReady && isDomContentLoaded) {
        ...
        if (window.addThreadGroupBtn) window.addThreadGroupBtn.addEventListener('click', () => openThreadGroupModal());
        if (window.cancelThreadGroupBtn) window.cancelThreadGroupBtn.addEventListener('click', closeThreadGroupModal);
        if (window.saveThreadGroupBtn) window.saveThreadGroupBtn.addEventListener('click', saveThreadGroup);
...
function renderThreadGroupsTable() {
    ...
    if (!threadGroups.length) {
        window.threadGroupsTableBody.innerHTML = '<tr><td colspan="3" class="text-gray-500 p-4">No groups yet.</td></tr>';
        return;
    }
...
function saveThreadGroup() {
    ...
    // Prevent duplicate group names - FIXED VERSION
    const nameLower = name.toLowerCase();
    const isDuplicate = threadGroups.some((g, idx) => {
        return g.name.toLowerCase() === nameLower && idx !== editingGroupIndex;
    });
    if (isDuplicate) {
